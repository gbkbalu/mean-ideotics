...
function VideoController(uiGridExporterConstants,uiGridExporterService,$timeout,$uibModal,$scope,$rootScope,$localStorage,$http ,$filter, $location, helpers ,EventService, UserService,VideoService,ProjectsService,CamerasService,AwsService) {
    ...
    $scope.gridOptions = {
        columnDefs: [
            ...
            { field: 'client' ,enableHiding:false, displayName: 'Project',
                cellTemplate: '<div class="ui-grid-cell-contents">{{grid.appScope.videoVM.showProject(row.entity)}}</div>'},
            { field:'cameraId', visible: true, displayName: 'Camera',
            ...
            { field: 'play' ,displayName: 'Play',enableHiding:false,
        ...
        if(vm.assignVideosList.length >0)
        {
            ...
            }else
            {
                if(vm.videosMysqlIds.length>0)
                {
                    ...
                    vm.fetchResultObject = {videoId:vm.videosMysqlIds,project:project,cameraId:selectedCamObj.cameraId,camera:selectedCamObj.camera._id
                        ,cameracode:selectedCamObj.camera.cameracode,clientsId:selectedCamObj.project.clientsId
