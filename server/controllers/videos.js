...
exports.updatecamvideos = function (req, res) {
    ...
    _.each(videosList, function (video) {
        mongoose.models.videos.update({videoId: video.videoId}, {$set: {startingTime: video.startingTime,
            dateOfTheVideo: video.dateOfTheVideo, endingTime:video.endingTime,endDateOfTheVideo:video.endDateOfTheVideo}}, function (err) {
...
exports.lockVideo = function (req, res) {
    ...
    mongoose.models.videos.findOne({ videoId: body.videoId }).populate("project").exec(function (err, video) {
        ...
        if (video.userId !== 0) {
            return res.status(200).json({ success: false, error: 'Video already locked by userId ' + video.userId });
        }
        ...
        if (video) {
            ...
            video.save(function (err, video) {
                if (!err) {
                    return res.status(200).json({ success: true, msg: 'Video Locked succesfully' });
                }
                else {
                    return res.status(500).json({ success: false, error: 'Something went wrong, Please try after some time' }, err);
                }
...
exports.saveVideosFromCSV = function (req, res) {
    ...
    mongoose.models.clients.find({}, function (err, icons) {
        ...
        for(var len=0;len<body.length;len++)
        {
            ...
            var eachBodyObj = {userId:0,videoId:bodyRec.videoId,client:bodyRec.client,
                name:bodyRec.name,url:bodyRec.url,status:0,camera:bodyRec.camera,project:clientObjIdArr[bodyRec.client],
                cameraId:bodyRec.cameraId,datasynced:0};
